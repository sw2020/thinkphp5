	...
	<form class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="" name="pics_name">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="" name="order">
			</div>
		</div>
		<!--<div class="row cl">
		  ...
		  <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>结束日期：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'}'})" id="datemax" class="input-text Wdate">
			</div>
		</div>-->
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片作者：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="" name="pic_author">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片来源：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="" name="pic_source">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="about" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp=""></textarea>
...
function article_save_submit(){
	$.ajax({
		url: '<?php echo url("Picture/addPicInfo"); ?>',
		method: 'POST',
		data: $("#form-article-add").serialize()+'&pic_id='+pic_id,
		success:function(data){
			if(data.status == 1){
				layer.msg(data.message,{icon:1,time:2000});
				setTimeout(function(){
					 //刷新页面
			        parent.window.location.href='<?php echo url("Picture/main"); ?>';
			        //获取窗口索引
...
(function( $ ){
    ...
    $(function() {
            ...
            if ( file.getStatus() === 'invalid' ) {
                ...
                uploader.makeThumb( file, function( error, src ) {
                    ...
                    if( isSupportBase64 ) {
                        ...
                        $.ajax('<?php echo url("Picture/upload"); ?>', {
                            method: 'POST',
                            data: src,
                            dataType:'json',
        ...
        $upload.on('click', function() {
    	   ...
    	   $.post('<?php echo url("Picture/upload"); ?>',{'pic_id':$("#pic_id").val()});
    	   if ( state === 'ready' ) {
               uploader.upload();
