...
class PHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new \think\Exception($error_message);
            }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new \think\Exception($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new \think\Exception($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new \think\Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new \think\Exception($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new \think\Exception($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new \think\Exception($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new \think\Exception($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw new \think\Exception($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new \think\Exception($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new \think\Exception($this->lang('authenticate'));
                        }
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new \think\Exception($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new \think\Exception($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new \think\Exception($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new \think\Exception($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!file_exists($path)) {
                throw new \think\Exception($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
